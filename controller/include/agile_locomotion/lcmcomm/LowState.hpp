...
#ifndef __lcmcomm_LowState_hpp__
#define __lcmcomm_LowState_hpp__
...
class LowState
{
    public:
        ...
        /// flag to distinguish high level or low level
        ...
        /// reference real-time from motion controller (unit: us)
...
int LowState::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->tick, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int LowState::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->tick, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
uint64_t LowState::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == LowState::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, LowState::getHash };

    uint64_t hash = 0x852053018f5a081cLL +
