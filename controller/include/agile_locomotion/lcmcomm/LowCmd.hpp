...
#ifndef __lcmcomm_LowCmd_hpp__
#define __lcmcomm_LowCmd_hpp__

#include <lcm/lcm_coretypes.h>

#include "lcmcomm/MotorCmd.hpp"
...
int LowCmd::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int LowCmd::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int LowCmd::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t LowCmd::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* LowCmd::getTypeName()
{
    return "LowCmd";
}

int LowCmd::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    for (int a0 = 0; a0 < 20; a0++) {
        tlen = this->motorCmd[a0]._encodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
...
int LowCmd::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    for (int a0 = 0; a0 < 20; a0++) {
        tlen = this->motorCmd[a0]._decodeNoHash(buf, offset + pos, maxlen - pos);
        if(tlen < 0) return tlen; else pos += tlen;
    }
...
int LowCmd::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    ...
    for (int a0 = 0; a0 < 20; a0++) {
        enc_size += this->motorCmd[a0]._getEncodedSizeNoHash();
    }
...
uint64_t LowCmd::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == LowCmd::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, LowCmd::getHash };

    uint64_t hash = 0x55f1520eeba5c538LL +
         lcmcomm::MotorCmd::_computeHash(&cp) +
